...
(function($){
    ...
    Highlighter.prototype.highlight = function() {
      ...
      var wordMatches = text.match(geturl);
      console.log('wordMatches', wordMatches);
var evaluated = [];
        $.each(wordMatches, function(index, match) {
          matches.push(match);
          if (evaluated.indexOf(match) === -1) {
            console.log(match);
            text = text.replace(new RegExp(match, 'g'), '<mark style="color:#3097D1;background-color:#fff">$&</mark>', 'g');
            evaluated.push(match);
          }
        });
